...
//get a byte from the file buffer st.
__inline unsigned char get8(io_oper* st) {
	unsigned char res = 0;
	st->Read(&res, 1);
	return res;
}
//get an unsigned short(big endding)
...
struct huffman_node{
	...
	unsigned char key;//value of key on the leaf-node
};
...
{//to store the huffman table in the jpeg file.
	unsigned char size[16];
		/*size[i] means "the number 
		 *of nodes with the depth of i in the huffman tree"*/
	int count;//
	unsigned char code[256];
	huffman_node* root;//pointer to the huffman tree generated by the 
};

typedef unsigned short qtab[64];//the quantization table
...
//generating of the huffman tree from the huffman table
void huffman_tree(huffman_table &table)
{
	init_node(table.root);
	huffman_node* previos[256];//expanded nodes
	huffman_node* current[256];//nodes need expanding
	int i, j, k, count=0, prev_size=1;//"count":next node need to encode
	previos[0]=table.root;

	for (i=0; i<16; i++){
		for (j=0; j<prev_size; j++){
			for (k=0; k<2; k++){//the expanding of a node
				init_node(previos[j]->child[k]);
				current[j*2+k]=previos[j]->child[k];
			}
		}
		for (j=0; j<table.size[i]; j++)//the key value of the leaf-node 
			current[j]->key=table.code[count++];
		if (count >= table.count) break;
		for (j=table.size[i], k=0; j<2*prev_size; j++)//refresh the nodes need expanding
			previos[k++]=current[j];
...
struct comp_info
{
	...
	int qt_id;//which quantization table the component uses
	int ac_table;
	int dc_table;
	int dc;/*the current dc value when decoding, initialize 0.
		    *(we must keep in mind that the dc is recorded as difference)
			*/
	int vw, vh;
};
//all informations of a jpeg picture
struct jpeg
{
	int w, h;//the width and height of the image 
	int type;

	int dri;//did the image define restart invert?
	int mcu_remain;

	comp_info comp[3];
	/*restore the informations of each component. take note that  
	 *three is the bound of the number of components.*/
	qtab qt[3];//restore the quantization table.
	huffman_table ac_huff[4], dc_huff[4];  //restore huffman tables.
	double G[64], covG[64];
		/*DCT: answer matrix = G * (source matrix) *covG
		 *IDCT:answer matrix = covG * (source matrix)* G   */
};
...
//get the length of the section and skip it.
static void skip_section(io_oper* st) {
	skip(st, get16be(st) - 2);
}

static void read_header(jpeg* j, io_oper* st) {
//start of frame(flag:FFC0)
	int end = st->Tell(); end += get16be(st);
		//locate the file position to the end of the section.
	get8(st);//accuracy of the image(usually 8)
	j->h = get16be(st);
	j->w = get16be(st);
	int t = get8(st); // number of components
	for (int i = 0; i < 3; ++i) {
		int id = get8(st);
